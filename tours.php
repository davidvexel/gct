...
$DCategoria = mysql_query($query_DCategoria, $conexion) or die(mysql_error());
$row_DCategoria = mysql_fetch_assoc($DCategoria);
$totalRows_DCategoria = mysql_num_rows($DCategoria);

$query_TCategoria = sprintf("SELECT * FROM tours AS T INNER JOIN localizacion AS L ON (T.localizacion_id = L.id_localizacion) INNER JOIN  categorias AS C ON (T.cat_id = C.id_cat) WHERE  T.cat_id = %s AND T.listado = 'SI' ORDER BY T.id_tour", GetSQLValueString($colname_id, "int"));
$query_limit_TCategoria = sprintf("%s LIMIT %d, %d", $query_TCategoria, $startRow_TCategoria, $maxRows_TCategoria);
$TCategoria = mysql_query($query_limit_TCategoria, $conexion) or die(mysql_error());
$row_TCategoria = mysql_fetch_assoc($TCategoria);

if (isset($_GET['totalRows_TCategoria'])) {
  $totalRows_TCategoria = $_GET['totalRows_TCategoria'];
} else {
  $all_TCategoria = mysql_query($query_TCategoria);
  $totalRows_TCategoria = mysql_num_rows($all_TCategoria);
}
...
<img src="<?php echo $path; ?>timthumb.php?src=/imagenes/categorias/<?php echo $row_DCategoria['imagen_cat']; ?>&w=220&h=200&zc=1&q=90" alt="<?php echo $row_DCategoria['nom_cat'.$idiomaDB]; ?>" title="<?php echo $row_DCategoria['nom_cat'.$idiomaDB]; ?>" class="imagen radius mbottom">
...
<h3 class="nomTour"><a class="ax" href="<?php echo $path; ?>detalles/<?php echo $row_TCategoria['id_tour']."-".urls_amigables($nom_tour).".php"; ?>" title=""><? echo $row_TCategoria['nom_tour'.$idiomaDB]; ?></a></h3>
...
<?php } while ($row_TCategoria = mysql_fetch_assoc($TCategoria)); }?>
...
<?php
mysql_free_result($DCategoria);
mysql_free_result($TCategoria);
?>