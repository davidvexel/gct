...
if (isset($_GET['id_cat'])) {
  $colname_busqueda = $_GET['id_cat'];
}
mysql_select_db($database_conexion, $conexion);
$query_busqueda = sprintf("SELECT T.id_tour, T.nom_tour, T.imagen, L.nom_localizacion, C.nom_cat FROM tours AS T INNER JOIN localizacion AS L ON (T.localizacion_id = L.id_localizacion) INNER JOIN  categorias AS C ON (T.cat_id = C.id_cat) WHERE  T.cat_id = %s ORDER BY T.id_tour", GetSQLValueString($colname_busqueda, "int"));
...
<td valign="top"> 
<h2 align="center">Categoría: <?php echo $row_busqueda['nom_cat']; ?></h2>
