...
$colname_busqueda = "-1";
if (isset($_GET['propiedad_destacada'])) {
  $colname_busqueda = $_GET['propiedad_destacada'];
}
mysql_select_db($database_conexion, $conexion);
$query_busqueda = sprintf("SELECT T.id_tour, T.nom_tour, T.imagen, L.nom_localizacion, C.nom_cat FROM tours AS T INNER JOIN localizacion AS L ON (T.localizacion_id = L.id_localizacion) INNER JOIN  categorias AS C ON (T.cat_id = C.id_cat) WHERE  T.destacado = %s ORDER BY T.id_tour", GetSQLValueString($colname_busqueda, "text"));
$query_limit_busqueda = sprintf("%s LIMIT %d, %d", $query_busqueda, $startRow_busqueda, $maxRows_busqueda);
$busqueda = mysql_query($query_limit_busqueda, $conexion) or die(mysql_error());
...
<td valign="top"> 
<h2 align="center">
<?php if(isset($_GET['propiedad_destacada']) && $_GET['propiedad_destacada'] == 'SI') { echo "Propiedades destacadas"; } ?>
<?php if(isset($_GET['propiedad_destacada']) && $_GET['propiedad_destacada'] == 'NO') { echo "Propiedades no destacadas"; } ?>
</h2>
