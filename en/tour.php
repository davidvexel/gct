...
$tipoCambio = mysql_query($query_tipoCambio, $conexion) or die(mysql_error());
$row_tipoCambio = mysql_fetch_assoc($tipoCambio);
$totalRows_tipoCambio = mysql_num_rows($tipoCambio);
?>
...
<ul class="datos">
        <?php if($row_tour['duracion'.$idiomaDB] != NULL) { ?><li>Duration:<span><?php echo $row_tour['duracion'.$idiomaDB]; ?></span></li> <?php } ?>
        <?php if($row_tour['temporada'.$idiomaDB] != NULL) { ?><li>Season:<span><?php echo $row_tour['temporada'.$idiomaDB]; ?></span></li> <?php } ?>
        <?php if($row_tour['disponibilidad'.$idiomaDB] != NULL) { ?><li>Availability:<span><?php echo $row_tour['disponibilidad'.$idiomaDB]; ?></span></li> <?php } ?>
        <?php if($row_tour['horario'.$idiomaDB] != NULL) { ?><li>Schedules:<span><?php echo $row_tour['horario'.$idiomaDB]; ?></span></li> <?php } ?>
        <?php if($row_tour['ideal_para'.$idiomaDB] != NULL) { ?><li>Ideal for:<span><?php echo $row_tour['ideal_para'.$idiomaDB]; ?></span></li> <?php } ?>
	    <?php if($row_tour['n_personas'.$idiomaDB] != NULL) { ?><li>Number of people:<span><?php echo $row_tour['n_personas'.$idiomaDB]; ?></span></li> <?php } ?>
		<?php if($row_tour['l_partida'.$idiomaDB] != NULL) { ?><li>Place of departure:<span><?php echo $row_tour['l_partida'.$idiomaDB]; ?></span></li><?php } ?>
		<?php if($row_tour['l_llegada'.$idiomaDB] != NULL) { ?><li>Arrive:<span><?php echo $row_tour['l_llegada'.$idiomaDB]; ?></span></li> <?php } ?>             
		<?php if($row_tour['r_adecuada'.$idiomaDB] != NULL) { ?><li>Suitable clothes:<span><?php echo $row_tour['r_adecuada'.$idiomaDB]; ?></span></li> <?php } ?>
        <?php if($row_tour['nom_localizacion'.$idiomaDB] != NULL) { ?><li>Location:<span> <?php echo $row_tour['nom_localizacion'.$idiomaDB]; ?></span></li> <?php } ?>
        <?php if($row_tour['nom_cat'.$idiomaDB] != NULL) { ?><li>Category:<span><?php echo $row_tour['nom_cat'.$idiomaDB]; ?></span> </li> <?php } ?>
	</ul>
...
$(document).ready(function(e) {
    ...
    $("#reservar").submit(function(e) {
		...
		if(error == false){	
			...
			$.post("<?php echo $path; ?>en/datos-sesion.php", v, function(data){ 
			$("#alerta").show();
			setTimeout(function(){document.location = '<?php echo $path; ?>en/reservar.php'},2000);
			}, "html");
		}	
	});
});
